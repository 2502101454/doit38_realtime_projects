 ...
 * 并把结果rdd利用新API指定outputFormat后，写入hbase
 */
object DataLoadJob01_GeoAreaInfo2Hbase {
  def main(args: Array[String]): Unit = {
    ...
    val df = spark.read.jdbc("jdbc:mysql://hadoop102:3306/wz_test", "t_md_areas", props)
    df.createTempView("t")
    ...
    val toHbaseRdd: RDD[(ImmutableBytesWritable, Put)] = resDf.rdd.map(row => {
      ...
      // 将本行数据封装到hbase的数据插入对象Put中，装入rowKey
    ...
    sc.hadoopConfiguration.set("hbase.zookeeper.quorum", "hadoop102")
